...
DopeBot.prototype.login = function (callback) {
  var self = this
  if (!this.spotify.expiresAt || this.spotify.expiresAt <= new Date()) {
    if (this.settings.code) {
      if (this.spotify.expiresAt === null) {
        this.logger.info('Retrieving an access token [Authorization code flow]')
        this.spotify.client.authorizationCodeGrant(this.settings.code)
        .then(function (data) {
          self.logger.info('The access token expires in ' + data.body['expires_in'])
          self.spotify.expiresAt = new Date()
          self.spotify.expiresAt.setTime(self.spotify.expiresAt.getTime() + data.body['expires_in'] * 1000)
          self.logger.info('The access token is ' + data.body['access_token'])
          self.logger.info('The refresh token is ' + data.body['refresh_token'])

          self.spotify.client.setAccessToken(data.body['access_token'])
          self.spotify.client.setRefreshToken(data.body['refresh_token'])

          callback()
        }, function (err) {
          self.logger.error('Something went wrong when retrieving an access token', err)
        })
      } else {
        this.logger.info('Refreshing access token [Authorization code flow]')
        this.spotify.client.refreshAccessToken()
        .then(function (data) {
          self.logger.info('The access token has been refreshed!')
          self.spotify.client.setAccessToken(data.body['access_token'])

          callback()
        }, function (err) {
          self.logger.error('Something went wrong when refreshing the access token', err)
        })
      }
    } else {
      this.logger.info('Retrieving an access token [Client Credential flow]')
      this.spotify.client.clientCredentialsGrant()
      .then(function (data) {
        self.logger.info('The access token expires in ' + data.body['expires_in'])
        self.spotify.expiresAt = new Date()
        self.spotify.expiresAt.setTime(self.spotify.expiresAt.getTime() + data.body['expires_in'] * 1000)
