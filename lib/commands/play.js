module.exports = {
  ...
  handler: async function (argv) {
    ...
    if (matches = argv.uri.match(/spotify:user:(\w+):playlist:([a-zA-Z0-9]{22})/)) {
      ...
      argv.uri = null
      argv.uris = []
      await handlePlaylist(argv, user, playlist)
      return
    }

    await addTracks(argv)
  }
}

async function addTracks(argv) {
  if (argv.db.state.state != 'playing') {
    ...
    await argv.db.mopidy.tracklist.add([null, null, argv.uri, argv.uris]);
    ...
    let tracks = await argv.db.mopidy.tracklist.add([null, null, argv.uri, argv.uris]);
...
async function handlePlaylist(argv, user, playlist) {
  if (!argv.db.search.expires || argv.db.search.expires <= new Date()) {
    ...
    .then(async function(data) {
      ...
      await lookupPlaylist(argv, user, playlist, 0)
    }
    , function(err) {
...
async function lookupPlaylist(argv, user, playlist, offset) {
  ...
  .then(async function(data) {
    ...
    if (argv.uris.length < data.body.total) {
      await lookupPlaylist(argv, user, playlist, offset+100)
    } else {
